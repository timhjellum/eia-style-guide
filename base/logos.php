<?php
require_once '../includes/variables.inc';
$globalTitle 	= 'U.S. Energy Information Administration (EIA)';
?>
<!doctype html>
<html>

<head>
	<?php include('../includes/head.inc'); ?>
</head>

<body>
	<?php include('../includes/header.inc'); ?>
	<main>
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<!-- $$$$$$$$$$  new section  $$$$$$$$$$ -->
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<section>
			<!-- start master layout-->
			<div class="grid-wrapper">
				<div class="grid-left">
					<!-- start left navigation-->
					<div></div>
					<!-- end master layout-->
				</div>
				<div class="grid-main">
					<!-- start main template area -->
					<div class="wrapper">

						<h1 id="introduction" class="header">
							<!-- title -->
						</h1>
						<p class="para"></p>
						<h2 class=" uk-h3">Header Logos</h2>
						<div class="eia-card header-logo">
							<h3 class="uk-h3 uk-text-muted">Example</h3>
							<div class="logo-full-size">
								<span class="logo"></span>
							</div>

							<h3 class="uk-h3 uk-text-muted">Shrunk Example</h3>
							<div class="logo-shrink">
								<span class="logo"></span>
							</div>
						</div>

					</div>
					<!-- end main template area -->
				</div>
				<div class="grid-right"></div>
			</div>
			<!-- end master layout-->
		</section>
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<!-- $$$$$$$$$$  new section  $$$$$$$$$$ -->
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<section>
			<!-- start master layout-->
			<div class="grid-wrapper">
				<div class="grid-left">
					<!-- start left navigation-->
					<div></div>
					<!-- end master layout-->
				</div>
				<div class="grid-main">
					<!-- start main template area -->
					<div class="wrapper">
						<h2 class="uk-h3">Header Logos Focus</h2>
						<!-- Example -->
						<div class="eia-card header-logo">
							<h3 class="uk-h3 uk-text-muted">Example</h3>
							<div class="logo-full-size">
								<span class="logo focus"></span>
							</div>

							<h3 class="uk-h3 uk-text-muted">Shrunk Example</h3>
							<div class="logo-shrink">
								<span class="logo focus"></span>
							</div>
						</div>
					</div>
					<!-- end main template area -->
				</div>
				<div class="grid-right"></div>
			</div>
			<!-- end master layout-->
		</section>
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<!-- $$$$$$$$$$  new section  $$$$$$$$$$ -->
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<section>
			<!-- start master layout-->
			<div class="grid-wrapper">
				<div class="grid-left">
					<!-- start left navigation-->
					<div></div>
					<!-- end master layout-->
				</div>
				<div class="grid-main">
					<!-- start main template area -->
					<div class="wrapper">
						<h2 class="uk-h3">Beta Header Logos</h2>
						<div class="eia-card header-logo">
							<h3 class="uk-h3 uk-text-muted">Example</h3>
							<div class="logo-full-size">
								<a class="logo beta"></a>
							</div>

							<h3 class="uk-h3 uk-text-muted">Shrunk Example</h3>
							<div class="logo-shrink">
								<a class="logo beta"></a>
							</div>
						</div>
					</div>
					<!-- end main template area -->
				</div>
				<div class="grid-right"></div>
			</div>
			<!-- end master layout-->
		</section>
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<!-- $$$$$$$$$$  new section  $$$$$$$$$$ -->
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<section>
			<!-- start master layout-->
			<div class="grid-wrapper">
				<div class="grid-left">
					<!-- start left navigation-->
					<div></div>
					<!-- end master layout-->
				</div>
				<div class="grid-main">
					<!-- start main template area -->
					<div class="wrapper">
						<h2 class="uk-h3">Beta Header Logos</h2>
						<div class="eia-card header-logo">
							<h3 class="uk-h3 uk-text-muted">Example</h3>
							<div class="logo-full-size">
								<a class="logo beta focus"></a>
							</div>

							<h3 class="uk-h3 uk-text-muted">Shrunk Example</h3>
							<div class="logo-shrink">
								<a class="logo beta focus"></a>
							</div>
						</div>
					</div>
					<!-- end main template area -->
				</div>
				<div class="grid-right"></div>
			</div>
			<!-- end master layout-->
		</section>
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<!-- $$$$$$$$$$  new section  $$$$$$$$$$ -->
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<section>
			<!-- start master layout-->
			<div class="grid-wrapper">
				<div class="grid-left">
					<!-- start left navigation-->
					<div></div>
					<!-- end master layout-->
				</div>
				<div class="grid-main">
					<!-- start main template area -->
					<div class="wrapper">
						<h2 class="uk-h3">Print Logo</h2>
						<div class="eia-card">
							<div class="uk-text-default uk-text-normal uk-text-muted">Example</div>
							<div class="logo-logo-white-background">
								<span class="print-logo"></span>
							</div>
						</div>
					</div>
					<!-- end main template area -->
				</div>
				<div class="grid-right"></div>
			</div>
			<!-- end master layout-->
		</section>
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<!-- $$$$$$$$$$  new section  $$$$$$$$$$ -->
		<!-- $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ -->
		<footer>
			<!-- start master layout-->
			<div class="grid-wrapper">
				<div class="grid-left">
					<!-- start left navigation-->
					<div><!-- left nav --></div>
					<!-- end master layout-->
				</div>
				<div class="grid-main">
					<!-- start main template area -->
					<h1>footer</h1>
					<!-- end main template area -->
				</div>
				<div class="grid-right"></div>
			</div>
			<!-- end master layout-->
		</footer>
	</main>
</body>

</html>